...
for ii = 1:size(ContactPoints,1) % loop over all contact points
    ...
    if Outcome.Gap < 0 % we have meaningful outcome from the search
        ...
        DOFpositions_cont = xlocChosen(DofsAtNode_cont, ContactNode_cont(ii),1:2);
        ...
        Ftarg(DOFpositions_targ) = Ftarg(DOFpositions_targ) + Nm_2412(xi,eta)' * Ftarg_loc; % redistribution over the nodes of target element 
        Fcont(DOFpositions_cont) = Fcont(DOFpositions_cont) + Fcont_loc;
    end       
end 
